...
class TestMatrix implements TestLifecycleLogger, TimeExecutionLogger, TestInterfaceDynamicTests {
    ...
    void testToString() {
        ...
        assertEquals("MATRIX=" +
                ...
                " [ 3.00 ] " +
                "}", mx.toString());
        mx = new Matrix(new double[][]{{1, 45.5}, {11.2, 67.43}, {3, 0.34}});
        assertEquals("MATRIX=" +
                "{" +
                " [ 1.00 45.50 ] " +
                " [ 11.20 67.43 ] " +
    ...
    void testScalarMultiply() {
        ...
        assertAll("Assertion all of these examples: ",
                ...
                () -> assertArrayEquals(new Array2DRowRealMatrix(finalMatrix1).scalarMultiply(0).getData(),
                        new Matrix(finalMatrix1).multiply(0).getMatrix()),
                () -> assertArrayEquals(new Array2DRowRealMatrix(finalMatrix2).scalarMultiply(-2.23).getData(),
    ...
    void testMultiplyByMatrixSuccess() {
        ...
        assertArrayEquals(new Array2DRowRealMatrix(multiplicand).multiply(new Array2DRowRealMatrix(multiplier)).getData(),
                new Matrix(multiplicand).multiply(new Matrix(multiplier)).getMatrix());
    }
    ...
    void testAddMatrixSuccess() {
        ...
        assertArrayEquals(new Array2DRowRealMatrix(augend).add(new Array2DRowRealMatrix(addend)).getData(),
    ...
    void testInverseMatrixException() {
        assertAll("Assertion all of these exceptions: ",
                () -> assertEquals("Only in square matrix determinant can be found!!!",
                        assertThrows(IllegalArgumentException.class, () -> new Matrix(new double[][]{
                                {1, 2},
                                {11.2, 48},
                                {3, 2.1}
                        }).findInvertibleMatrix()).getMessage()),
                () -> assertEquals("The matrix is singular (degenerate) - вироджена",
                        assertThrows(IllegalArgumentException.class, () -> new Matrix(new double[][]{
                                {1, 2, 3},
                                {4, 5, 6},
                                {5, 7, 9}
